...
module OpenSRS
  ...
  class Server
    attr_accessor :server, :username, :password, :key, :proxy
    ...
    def http
      if proxy.present?  
        p = URI.parse(proxy)
        http = Net::HTTP::Proxy(p.host, p.port, p.user, p.password).start(server.host, server.port, :use_ssl => (server.scheme == "https"), :verify_mode => OpenSSL::SSL::VERIFY_NONE)
      else
